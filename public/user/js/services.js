	...
	.factory('routerUtils', ['$state', 'spaces', function($state, spaces){
		...
		fn.goToSearch = function(){
 		...
 		fn.getThumb = function(contributionId, attachment){
            ...
            if(attachment.thumb){
              path.image = true; 
              path.style = {"background-image": "url(" + "/api/contributions/" + contributionId + /attachments/+ attachment.id + "/thumbnail" + ")" ,  "background-repeat": "no-repeat", "background-size" : "cover"};
            }
            else{
